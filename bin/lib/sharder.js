#!/usr/bin/env node

/**
 * sharder.js — Atomic Artifacts (Sharding) for large PRDs and specs.
 * 
 * Part of Jump Start Framework (Item 8: Atomic Artifacts).
 * 
 * Breaks large PRDs into smaller shards to avoid context saturation.
 * Each shard covers a domain or epic; a parent index maintains cross-links.
 */

'use strict';

const fs = require('fs');
const path = require('path');

/**
 * Extract epics from a PRD file.
 * 
 * @param {string} content - PRD markdown content.
 * @returns {object[]} Array of { id, name, content, storyCount } objects.
 */
function extractEpics(content) {
  const epicPattern = /### Epic (E\d+):\s*(.+)[\s\S]*?(?=### Epic E\d+:|## Non-Functional|---\s*\n## |$)/g;
  const epics = [];
  let match;
  
  while ((match = epicPattern.exec(content)) !== null) {
    const epicContent = match[0];
    const storyMatches = epicContent.match(/#### Story E\d+-S\d+/g);
    epics.push({
      id: match[1],
      name: match[2].trim(),
      content: epicContent,
      storyCount: storyMatches ? storyMatches.length : 0
    });
  }
  
  return epics;
}

/**
 * Determine if a PRD should be sharded based on size heuristics.
 * 
 * @param {string} content - PRD markdown content.
 * @param {object} [thresholds] - Custom thresholds.
 * @returns {{ shouldShard: boolean, reason: string, epicCount: number, storyCount: number, lineCount: number }}
 */
function shouldShard(content, thresholds = {}) {
  const {
    maxEpics = 5,
    maxStories = 15,
    maxLines = 800
  } = thresholds;
  
  const epics = extractEpics(content);
  const totalStories = epics.reduce((sum, e) => sum + e.storyCount, 0);
  const lineCount = content.split('\n').length;
  
  const reasons = [];
  if (epics.length > maxEpics) reasons.push(`${epics.length} epics (max ${maxEpics})`);
  if (totalStories > maxStories) reasons.push(`${totalStories} stories (max ${maxStories})`);
  if (lineCount > maxLines) reasons.push(`${lineCount} lines (max ${maxLines})`);
  
  return {
    shouldShard: reasons.length > 0,
    reason: reasons.length > 0 ? reasons.join('; ') : 'Within limits',
    epicCount: epics.length,
    storyCount: totalStories,
    lineCount
  };
}

/**
 * Generate a PRD shard for a specific epic or group of epics.
 * 
 * @param {string} parentPrdContent - Full PRD content.
 * @param {string[]} epicIds - Epic IDs to include in this shard.
 * @param {number} shardIndex - Shard number.
 * @returns {string} Markdown content for the shard.
 */
function generateShard(parentPrdContent, epicIds, shardIndex) {
  const epics = extractEpics(parentPrdContent);
  const shardEpics = epics.filter(e => epicIds.includes(e.id));
  
  if (shardEpics.length === 0) return '';
  
  const header = `# PRD Shard ${shardIndex}: ${shardEpics.map(e => `${e.id} — ${e.name}`).join(', ')}

> **Parent Document:** [specs/prd.md](../prd.md)
> **Shard:** ${shardIndex} of N
> **Epics:** ${epicIds.join(', ')}
> **Status:** Draft

---

`;
  
  const body = shardEpics.map(e => e.content).join('\n\n---\n\n');
  
  return header + body;
}

/**
 * Generate a PRD index that links all shards.
 * 
 * @param {object[]} shards - Array of { index, epicIds, filePath } objects.
 * @param {string} projectName - Project name for the header.
 * @returns {string} Markdown index content.
 */
function generateIndex(shards, projectName) {
  let content = `# PRD Index — ${projectName || 'Project'}

> **Purpose:** Master index for sharded PRD documents.
> **Total Shards:** ${shards.length}

---

## Shard Map

| Shard | Epics | File | Status |
|-------|-------|------|--------|
`;
  
  for (const shard of shards) {
    const epicList = shard.epicIds.join(', ');
    content += `| ${shard.index} | ${epicList} | [${shard.filePath}](${shard.filePath}) | Draft |\n`;
  }
  
  content += `
---

## Cross-References

<!-- Auto-generated cross-reference links between shards -->

`;
  
  return content;
}

module.exports = {
  extractEpics,
  shouldShard,
  generateShard,
  generateIndex
};
